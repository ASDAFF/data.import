...
class CDataImport
{
	...
	public static function CheckEntitiesNames($PLAN_ID = false, $entities = Array()) : array
	{
		...
		$planCData = new CDataImportPlanConnections;
	...
	private static function getActiveConnectionsCount($PLAN_ID = false) : int
	{
		...
		if($PLAN_ID > 0)
		{
			$CConnection = new CDataImportPlanConnections;
	...
	{
		$resultText = "CDataImport::Load({$PLAN_ID});";
		
		$planData = new CDataImportPlan;
		...
		CDataImportUtils::replaceMacroses($IMPORT_FILE_URL, $planParams);
		/*
	...
	public static function Import($PLAN_ID = false, $startFrom = 0, $endTo = false, $IMPORT_FILE = false)
	{
		...
		if(self::DEBUG_MEMORY)
			CDataCoreFunctions::dump(['START IMPORT', CDataCoreFunctions::ConvertFileSize(memory_get_usage())]);
		...
		if($rsModule == MODULE_DEMO_EXPIRED)
		{
			$errorArray = Array(
				...
				"TAG" => "DATA_IMPORT_MODULE_DEMO_EXPIRED",
				"MODULE_ID" => self::MODULE_ID,
				"ENABLE_CLOSE" => "Y"
			);
			$notifyID = CAdminNotify::Add($errorArray);
		...
		$logData = new CDataImportLog;
		$logFields = Array("PLAN_ID" => $PLAN_ID);
		
		$resultText = "CDataImport::Import({$PLAN_ID});";
		...
		if(self::DEBUG_MEMORY)
			CDataCoreFunctions::dump(['GET CONNECTIONS', CDataCoreFunctions::ConvertFileSize(memory_get_usage())]);
		...
		if(self::DEBUG_MEMORY)
			CDataCoreFunctions::dump(['GET ENTITIES', CDataCoreFunctions::ConvertFileSize(memory_get_usage())]);
		...
		if(self::DEBUG_MEMORY)
			CDataCoreFunctions::dump(['GET DATA ARRAY', CDataCoreFunctions::ConvertFileSize(memory_get_usage())]);
		...
		{
			$resultText = "CDataImport::Import({$PLAN_ID}, {$fileDataArrayRes["START_FROM"]});";
		}
		
		if($startFrom == 0)
		{
			$searchIdData = new CDataImportPlanSearchId;
		...
		if($fileDataArrayRes["FINISHED"])
		{
			...
			{
				$clearImagesDir = CDataImportFinishEvents::DeleteDirFilesOnly($planParams["PATH_TO_IMAGES"]);
				if($DEBUG_EVENTS)
				{
			...
			if($planParams["CLEAR_UPLOAD_TMP_DIR"] == "Y")
			{
				...
				$clearUploadTmpDir = CDataImportFinishEvents::DeleteDirFilesOnly("/{$upload_dir}/tmp/");
				if($DEBUG_EVENTS)
				{
			...
			{
				$clearFilesDir = CDataImportFinishEvents::DeleteDirFilesOnly($planParams["PATH_TO_FILES"]);
				if($DEBUG_EVENTS)
				{
			...
			if($planParams["IMPORT_IBLOCK_SECTIONS"] == "Y")
			{
				if($planParams["SECTIONS_OUT_ACTION"] != "N" || $planParams["SECTIONS_IN_ACTION"] != "N")
				{
					...
					if(count($searchSectionIdList) > 0)
					{
						...
						{
							$changedOutSections = CDataImportFinishEvents::ChangeOutSections(
								$searchSectionIdList,
						...
						{
							$changedInSections = CDataImportFinishEvents::ChangeInSections(
								$searchSectionIdList,
			...
			if($planParams["IMPORT_IBLOCK_ELEMENTS"] == "Y")
			{
				if($planParams["ELEMENTS_OUT_ACTION"] != "N" || $planParams["ELEMENTS_IN_ACTION"] != "N")
				{
					...
					if(count($searchElementIdList) > 0)
					{
						...
						{
							$changedOutElements = CDataImportFinishEvents::ChangeOutElements(
								$searchElementIdList,
						...
						{
							$changedInElements = CDataImportFinishEvents::ChangeInElements(
								$searchElementIdList,
			...
			if($planParams["IMPORT_CATALOG_PRODUCT_OFFERS"] == "Y")
			{
				if($planParams["OFFERS_OUT_ACTION"] != "N" || $planParams["OFFERS_IN_ACTION"] != "N")
				{
					...
					if(count($searchOfferIdList) > 0)
					{
						...
						{
							$changedOutOffers = CDataImportFinishEvents::ChangeOutElements(
								$searchOfferIdList,
						...
						{
							$changedInOffers = CDataImportFinishEvents::ChangeInElements(
								$searchOfferIdList,
			...
			if($planParams["PROCESSINGS_AFTER_FINISH"] != '' && CModule::IncludeModule('data.massprocessing'))
			{
				...
				{
					$cProcessing = new \Data\MassProcessing\Processing;
					$CRunner = new \Data\MassProcessing\Runner;
		...
		if(self::DEBUG_EXECUTION_TIME)
			CDataCoreFunctions::dump(['SCRIPT EXECUTION TIME', round(microtime(true) - $scriptExecutionTime, 4).' s.']);
		
		if(self::DEBUG_MEMORY)
			CDataCoreFunctions::dump(['FINISH IMPORT', CDataCoreFunctions::ConvertFileSize(memory_get_usage())]);
		
		$collectedCycles = gc_collect_cycles();
		
		if(self::DEBUG_COLLECT_CYCLES)
			CDataCoreFunctions::dump(['COLLECTED CYCLES', $collectedCycles]);
	...
	private static function GetFileArrayByUrlMulti($urls = false, $value = false, $useWhatermark = false, $whaterMarkParams = Array(), &$params)
	{
		...
		elseif(is_array($urls))
		{
			...
			foreach ($multiCurl as $k => $ch) {
				$temp = tempnam(sys_get_temp_dir(), 'data.import');
				$handle = fopen($temp, "w");
	...
	private static function GetFileArrayByUrl($url = '', $property = false, $value = false, $useWhatermark = false, $whaterMarkParams = Array(), &$params)
	{
		...
		if($url && extension_loaded('curl'))
		{
			...
			if($httpCode == 200 || ($httpCode == 226 && strtolower($urlParams["scheme"]) == 'ftp'))
			{
				$tempFile = tempnam(sys_get_temp_dir(), 'data.import');
	...
	private static function CheckFields(&$fields, $params, &$additionalCode = false)
	{
		...
		if($codeParams["TRANSLITERATION"] == "Y")
		{
			...
			{
				uasort($additionalCode, 'CDataImportUtils::CompareCodeSort');
			...
			if($translitName)
			{
				$fields["CODE"] = CDataImportUtils::TranslitValue($translitName, $codeParams["TRANS_SPACE"], $codeParams["TRANS_OTHER"], $codeParams["TRANS_LEN"], $codeParams["TRANS_CASE"]);
			}
		}
	}
		...
		if(($rule["IS_IMAGE"] == "Y" || $rule["IS_FILE"] == "Y") && $rule["IS_URL"] != "Y")
		{
			if($rule["IS_IMAGE"] == "Y")
			{
				if(is_array($field) && count($field))
				{
					...
					{
						$fileArr = self::GetFileArrayByFileName($value, $params["PATH_TO_IMAGES"], CDataImportProperty::GetPropertyMultiply($rule[$checkRule]), $fileArr, $params["PROPERTIES_WHATERMARK"], $whaterMarkParams, $DEBUG_IMAGES, $params);
					}
				}
				else
				{
					$fileArr = self::GetFileArrayByFileName($field, $params["PATH_TO_IMAGES"], CDataImportProperty::GetPropertyMultiply($rule[$checkRule]), $resultFields[$rule[$checkRule]], $params["PROPERTIES_WHATERMARK"], $whaterMarkParams, $DEBUG_IMAGES, $params);
				}
			...
			elseif($rule["IS_FILE"] == "Y")
			{
				if(is_array($field) && count($field))
				{
					foreach($field as $value)
					{
						$fileArr = self::GetFileArrayByFileName($value, $params["PATH_TO_FILES"], CDataImportProperty::GetPropertyMultiply($rule[$checkRule]), $fileArr, $params["PROPERTIES_WHATERMARK"], Array(), $DEBUG_FILES, $params);
					}
				}
				else
				{
					$fileArr = self::GetFileArrayByFileName($field, $params["PATH_TO_FILES"], CDataImportProperty::GetPropertyMultiply($rule[$checkRule]), $resultFields[$rule[$checkRule]], $params["PROPERTIES_WHATERMARK"], Array(), $DEBUG_FILES, $params);
				}
	...
	private static function GetSectionArray($item, &$rules, &$params, $defaultParams)
	{
		...
		foreach($item as $entity => $field)
		{
			...
			if(isset($rules[$entity]))
			{
				...
				{
					/*if($params['IMPORT_FORMAT'] == 'XML')
						CDataImportUtils::CheckAttribute($rule, $temp_field, $field);*/
					CDataImportUtils::CheckAttribute($rule, $temp_field, $field);
				
					CDataImportProcessingSettingsTypes::ApplyProcessingRules($field, $rule["PROCESSING_TYPES"]);
					
					$CheckRequired = CDataImportUtils::CheckRequired($rule, $field);
					...
					$CheckArrayContinue = CDataImportUtils::CheckArrayContinue($rule, $field);
					...
					if(!empty($rule["IBLOCK_SECTION_PARENT_FIELD"]) || !empty($rule["IBLOCK_SECTION_FIELD"]))
					{
						...
						)
						{
							...
							if(($rule["IS_REQUIRED"] == "N" && !empty($field)) || ($rule["IS_REQUIRED"] == "Y" || !empty($field)))
							{
								...
								$sectionParentFields[$rule["IBLOCK_SECTION_DEPTH_LEVEL"]][$rule["IBLOCK_SECTION_PARENT_FIELD"]] = CDataImportUtils::ClearField($field);
						...
						)
						{
							...
							if(($rule["IS_REQUIRED"] == "N" && !empty($field)) || ($rule["IS_REQUIRED"] == "Y" || !empty($field)))
							{
								...
								if($CheckImageFileUrl === false)
								{
									...
									{
										$sectionFields[$rule["IBLOCK_SECTION_DEPTH_LEVEL"]]["IPROPERTY_TEMPLATES"][$rule["IBLOCK_SECTION_FIELD"]] = CDataImportUtils::ClearField($field);
									}
									if(is_array($sectionFields[$rule["IBLOCK_SECTION_DEPTH_LEVEL"]][$rule["IBLOCK_SECTION_FIELD"]]))
										$sectionFields[$rule["IBLOCK_SECTION_DEPTH_LEVEL"]][$rule["IBLOCK_SECTION_FIELD"]][] = CDataImportUtils::ClearField($field);
									else
										$sectionFields[$rule["IBLOCK_SECTION_DEPTH_LEVEL"]][$rule["IBLOCK_SECTION_FIELD"]] = CDataImportUtils::ClearField($field);
								}
					...
					if(!empty($rule["IBLOCK_SECTION_FIELD"]) || !empty($rule["IBLOCK_ELEMENT_FIELD"]) || !empty($rule["IBLOCK_ELEMENT_PROPERTY"]) || $rule["USE_IN_CODE"] == 'ELEMENT')
					{
						if(!empty($rule["IBLOCK_ELEMENT_FIELD"]))
						{
							if($rule["IS_REQUIRED"] == "N" || ($rule["IS_REQUIRED"] == "Y" || !empty($field)))
							{
								...
								if($CheckImageFileUrl === false)
								{
									...
									{
										$elementFields["IPROPERTY_TEMPLATES"][$rule["IBLOCK_ELEMENT_FIELD"]] = CDataImportUtils::ClearField($field);
									}
									$elementFields[$rule["IBLOCK_ELEMENT_FIELD"]] = CDataImportUtils::ClearField($field);
								}
						...
						if(!empty($rule["IBLOCK_ELEMENT_PROPERTY"]))
						{
							if($rule["IS_REQUIRED"] == "N" || ($rule["IS_REQUIRED"] == "Y" || !empty($field)))
							{
								...
								$elementFields["PROPERTY_VALUES"][$rule["IBLOCK_ELEMENT_PROPERTY"]] = CDataImportUtils::ClearField($field);
	...
	private static function GetElementOfferArray($item, &$rules, &$params, $ELEMENT_ID = false, &$defaultParams, &$whaterMarkParams)
	{
		...
		if($params["PROPERTIES_SET_DEFAULT_VALUES"] == "Y" && CModule::IncludeModule("data.core"))
			$elementFields["PROPERTY_VALUES"] = CDataCoreIblock::GetDefaultProperties($params["OFFERS_IBLOCK_ID"], ["USE_LIST_VALUE" => "Y", "USE_HTML_TEXT" => "Y"]);
					...
					if(!empty($rule["IBLOCK_ELEMENT_OFFER_FIELD"]) || !empty($rule["IBLOCK_ELEMENT_OFFER_PROPERTY"]))
					{
						...
						if(!empty($rule["IBLOCK_ELEMENT_OFFER_FIELD"]))
						{
							...
							if($rule["IS_REQUIRED"] == "N" || ($rule["IS_REQUIRED"] == "Y" || !empty($field)))
							{
								...
								if($CheckImageFileUrl === false)
								{
									$elementFields[$rule["IBLOCK_ELEMENT_OFFER_FIELD"]] = CDataImportUtils::ClearField($field);
								}
						...
						if(!empty($rule["IBLOCK_ELEMENT_OFFER_PROPERTY"]))
						{
							...
							if($rule["IS_REQUIRED"] == "N" || ($rule["IS_REQUIRED"] == "Y" || !empty($field)))
							{
								...
								{
									CDataImportProperty::LinkPropertyValue($rule["IBLOCK_ELEMENT_OFFER_PROPERTY"], $field, $ID, $params["OFFERS_IBLOCK_ID"], $params, $data, $elementFields["PROPERTY_VALUES"][$rule["IBLOCK_ELEMENT_OFFER_PROPERTY"]]);
								}
		...
		if(count($elementFields)>0)
		{
			...
			if($ELEMENT_ID>0)
			{
				...
				{
					$elementFields["NAME"] = CDataImportElement::GetElementNameByID($ELEMENT_ID, $params);
				}
			}
		}
	...
	private static function ImportElement($data, &$params)
	{
		...
		{
			$arPreFilter = CDataImportFilter::SerializePreFilter($params['ELEMENTS_PREFILTER']);
	...
	{
		if($params["PROPERTIES_SET_DEFAULT_VALUES"] == "Y" && CModule::IncludeModule("data.core"))
			$elementProperties = CDataCoreIblock::GetDefaultProperties($IBLOCK_ID, ["USE_LIST_VALUE" => "Y", "SKIP_HTML" => "Y", "USE_HTML_TEXT" => "Y"]);
		else
				...
				foreach($rules[$entity]["RULES"] as $rule)
				{
					if(!empty($rule[$CONNECTION_TYPE]))
					{
						...
						if($rule["IS_REQUIRED"] == "N" || ($rule["IS_REQUIRED"] == "Y" && !empty($field)))
						{
							...
							if($CheckMap === false && $CheckImageFileUrlForProperty === false)
							{
								$elementProperties[$rule[$CONNECTION_TYPE]] = CDataImportUtils::ClearField($field);
							}
		...
		{
			$presetProperties = CDataImportProperty::PresetPropertiesByParam($item["param_array"], $params, $IBLOCK_ID);
	...
	private static function ImportProperties($data, &$params, $ELEMENT_ID = false, $IBLOCK_ID = false)
	{
		...
		if($DEBUG_EVENTS && $DEBUG_IMPORT_PROPERTIES)
		{
			$logProperties = new CDataImportLog;
		...
		if($DEBUG_EVENTS && $DEBUG_IMPORT_PROPERTIES)
		{
			if($ID > 0)
			{
				...
				$logPropertiesData["DATA"] = base64_encode(serialize(CDataImportProperty::getPropertiesIdAndValue($findProperties)));
				$logProperties->Add($logPropertiesData);
			}
			elseif(!$ID)
			{
		...
		if($ID > 0)
		{
			if(count($arDeleteValueProperty)>0)
			{
				...
				{
					CDataImportProperty::DeleteOldFilesByProperty($ID, $IBLOCK_ID, $propertyId);
				}
			}
			
			foreach($arProperties as $code => $value)
			{
				CDataImportProperty::LinkPropertyValue($code, $value, $ID, $IBLOCK_ID, $params, $data, $arPropertyValues[$code]);
			}
	...
	private static function ImportProduct($data, &$params, $type = "PRODUCTS")
	{
		...
		if($DEBUG_EVENTS && $DEBUG_IMPORT_PRODUCTS)
		{
			$logProducts = new CDataImportLog;
	...
	private static function GetPriceArray($item, &$rules, &$params, $ELEMENT_ID = false, $BASE_PRICE = false)
	{
		...
		{
			$prices[$BASE_PRICE]["PRICE"] = CDataImportUtils::ClearPrice($priceFields["PRICE"], true);
		...
		while ($arPriceType = $dbPriceType->Fetch())
		{
			...
			if(isset($priceFields["PRICE_".$PRICE_ID]) || isset($priceFields["EXTRA_ID_".$PRICE_ID]))
			{
				...
				{
					$prices[$PRICE_ID]["PRICE"] = CDataImportUtils::ClearPrice($priceFields["PRICE_".$PRICE_ID], true);
	...
	private static function GetStoreAmountArray($item, &$rules, &$params, $ELEMENT_ID = false)
	{
		...
		while ($arStore = $dbStore->Fetch())
		{
			...
			{
				$stores[$STORE_ID]["AMOUNT"] = CDataImportUtils::ClearPrice($storeAmountFields["STORE_".$STORE_ID]);
	...
	private static function GetEntityArray($item, &$rules, &$params) : array
	{
				...
				foreach($rules[$entity]["RULES"] as $rule)
				{
					if(!empty($rule["HIGHLOAD_BLOCK_ENTITY_FIELD"]))
					{
						...
						if($rule["IS_REQUIRED"] == "N" || ($rule["IS_REQUIRED"] == "Y" || !empty($field)))
						{
							...
							if($CheckImageFileUrl === false)
							{
								$entityFields[$rule["HIGHLOAD_BLOCK_ENTITY_FIELD"]] = CDataImportUtils::ClearField($field);
							}
		...
		{
			uasort($entityCode, 'CDataImportUtils::CompareCodeSort');
		...
		if($translitName)
		{
			$xml_id = CDataImportUtils::TranslitValue($translitName, "_", "_");
	...
	private static function GetUserFieldEnumByValue(int $userFieldID = NULL, $value, &$params) : string
	{
		...
		if(!empty($value) && $userFieldID)
		{
			...
			{
				$xml_id = CDataImportUtils::TranslitValue($value, "_", "_");
			}
