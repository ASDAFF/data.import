...
class CDataImportProperty
{
	...
	public static function PresetPropertiesByParam($data = [], &$params, $IBLOCK_ID = false)
	{
		...
		{
			$itemName = ($item["CODE"] != '' ? $item["CODE"] : CDataImportUtils::ClearField($item["NAME"]));
			if($itemName)
			{
				if(is_array($newData) && array_key_exists($itemName, $newData))
				{
					...
					$newData[$itemName]['VALUE'][] = CDataImportUtils::ClearField($item['VALUE']);
					$newData[$itemName]['UNIT'][] = CDataImportUtils::ClearField($item['UNIT']);
				}
				else
				{
		...
		{
			$itemName = CDataImportUtils::ClearField($item["NAME"]);
			$itemUnit = $item["UNIT"];
			...
			if(!$PROPERTY_ID && $params["XML_ADD_PROPERTIES_FOR_PARAMS"] == "Y")
			{
				...
				{
					$CODE = CDataImportUtils::TranslitValue($itemName, "_", "");
			...
			switch($propertyArray["PROPERTY_TYPE"])
			{
				case("S"):
					if($propertyArray["USER_TYPE"] == "directory")
					{
						...
						$value = CDataImportHighload::GetHighloadblockIdByValue($highloadblockTable, $value, $params);
					}
				...
				case("L"):
					...
					break;
				case("E"):
					$value = CDataImportElement::GetElementIdByField($propertyArray["LINK_IBLOCK_ID"], $data["SEARCH_PROPERTY_E"][$code], $value, $params["PROPERTIES_ADD_LINK_ELEMENT"]);
					break;
				case("G"):
					if($value)
					{
						$value = CDataImportSection::GetSectionIdByField(
							Array(
	...
	public static function GetPropertyIdByValue($IBLOCK_ID, $code, $value, &$params, $sort = 500)
	{
		...
		if(!empty($value) && !empty($code))
		{
			foreach($value as $sValue)
			{
				...
				$sValue = CDataImportUtils::ClearField($sValue);
				...
				{
					$xml_id = CDataImportUtils::TranslitValue($sValue, "-", "-");
				}
