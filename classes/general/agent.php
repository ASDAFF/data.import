...
class CDataImportAgent
{
	...
	public static function SearchAgent($PLAN_ID = false, $TYPE = "Import", $is_active = false)
	{
		...
		if($PLAN_ID>0)
		{
			$query = "SELECT * FROM `b_agent` WHERE `NAME` LIKE '%CDataImport::{$TYPE}({$PLAN_ID}%'";
		}
		else
		{
			$query = "SELECT * FROM `b_agent` WHERE `NAME` LIKE '%CDataImport::{$TYPE}%'";
			if($is_active !== false)
			{
