...
if($back_url=='')
	$back_url = "data.import_plans.php?lang=".LANG;
...
if($REQUEST_METHOD == "POST" && ($save!="" || $apply!="") && $moduleAccessLevel=="W" && check_bitrix_sessid()) 
{
	...
	{
		if ($apply != "")
			LocalRedirect("/bitrix/admin/data.import_plan_edit.php?ID=".$ID."&mess=ok&lang=".LANG."&".$tabControl->ActiveTabParam());
		else
			LocalRedirect("/bitrix/admin/data.import_plans.php?lang=".LANG);
	}
	else
	{
...
if($str_AGENT_ID>0)
{
	...
	if(is_array($agentArr))
	{
		if($agentArr['ACTIVE'] == 'N')
			$strWarning .= Loc::getMessage("DATA_IMPORT_AGENT_DEACTIVTED");
	}
	else
		$strWarning .= Loc::getMessage("DATA_IMPORT_AGENT_DELETED");
}
elseif($str_ACTIVE == 'Y')
	$strWarning .= Loc::getMessage("DATA_IMPORT_AGENT_DELETED");
...
$aMenu = array(
	array(
		...
		"LINK"=>"data.import_plans.php?lang=".LANG,
		"ICON"=>"btn_list",
	)
);
...
if($ID>0)
{
	...
	if($moduleAccessLevel=="W")
	{
		$aMenu[] = array(
			...
			"LINK"  => "data.import_connections_import.php?lang=".LANGUAGE_ID."&PLAN_ID=".$ID,
		);
	}
	...
	if($moduleAccessLevel >= "T")
	{
		$aMenuActions[] = array(
			...
			"LINK"  => "data.import_manually.php?PLAN_ID=".$ID."&LOAD_FILES=Y&lang=".LANG."&".bitrix_sessid_get(),
			"LINK_PARAM"  => "target=\"_blank\"",
		);
	}
	$aMenuActions[] = array(
		"TEXT"  => Loc::getMessage("OPEN_LOGS"),
		"LINK"  => "data.import_logs.php?PLAN_ID=".$ID."&find_plan_id=".$ID."&apply_filter=Y&lang=".LANG,
		"LINK_PARAM"  => "target=\"_blank\"",
	);
	if($moduleAccessLevel=="W")
	{
		$aMenuActions[] = array(
			...
			"LINK"  => "data.import_plan_edit.php?lang=".LANG,
			"ICON"  => "edit",
		);
		$aMenuActions[] = array(
			"TEXT"  => Loc::getMessage("COPY_BTN"),
			"TITLE" => Loc::getMessage("COPY_BTN_TITLE"),
			"LINK"  => "data.import_plan_edit.php?COPY_ID={$ID}&lang=".LANG,
			"ICON"  => "copy",
		);
	}
	$aMenuActions[] = array(
	...
	if($moduleAccessLevel=="W")
	{
		$aMenuActions[] = array(
			...
			"LINK"  => "javascript:if(confirm('".Loc::getMessage("ELEMENT_DELETE_BTN_MESSAGE")."')) ".
			  "window.location='data.import_plans.php?action=delete&ID[]=".CUtil::JSEscape($ID)."&lang=".LANGUAGE_ID."&".bitrix_sessid_get()."';",
			"ICON"  => "delete",
		);
	}
...
if($strWarning)
{
	CDataCoreFunctions::showAlertBegin('warning', 'warning');
	echo $strWarning;
	CDataCoreFunctions::showAlertEnd();
}
if($message)
...
$arFormFields["MAIN"]["ITEMS"][] = Array(
	...
	"ITEMS" => CDataImportPlan::getFormats(),
);
$arFormFields["MAIN"]["ITEMS"][] = Array(
...
$arFormFields["FILES"]["ITEMS"][] = Array(
	...
	"PARAMS" => Array(
		...
		"SELECT" => "D",
		"ALLOW_UPLOAD" => "N",
		"PATH" => "/".COption::GetOptionString("main", "upload_dir", "upload")."/data.import/data/images/",
		"PLACEHOLDER" => "/upload/data.import/data/images/",
	)
);
$arFormFields["FILES"]["ITEMS"][] = Array(
...
$arFormFields["FILES"]["ITEMS"][] = Array(
	...
	"PARAMS" => Array(
		...
		"SELECT" => "D",
		"ALLOW_UPLOAD" => "N",
		"PATH" => "/".COption::GetOptionString("main", "upload_dir", "upload")."/data.import/data/files/",
		"PLACEHOLDER" => "/upload/data.import/data/files/",
	)
);
$arFormFields["FILES"]["ITEMS"][] = Array(
...
if(!Loader::includeModule('data.massprocessing'))
{
	$arFormFields["POST_PROCESSING"]["ITEMS"][] = Array(
		"CODE" => "DATA_MASSPROCESSING_NOTE",
		"TYPE" => "NOTE",
		"VALUE" => Loc::getMessage(
			"DATA_MASSPROCESSING_NOTE",
			[
...
{
	$massProcessingsArr = [];
	$processingClass = new \Data\MassProcessing\Processing();
