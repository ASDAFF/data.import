...
$importFormats = CDataImportPlan::getFormats();
$fileCharsets = Array(
	"UTF-8" => "UTF-8",
...
{
	$plan_edit_link = "data.import_plan_edit.php?ID=".$f_ID."&lang=".LANG;
	...
	if($f_AGENT_ID>0)
	{
		...
		if(is_array($agentArr))
		{
			...
			if($agentArr['ACTIVE'] == 'N')
				$agentViewField .= '<img src="/bitrix/panel/data.ozon/theme/images/yellow.gif" title="'.Loc::getMessage("DATA_IMPORT_AGENT_DEACTIVTED").'" />&nbsp;';
			$agentViewField .= '<a class="ui-btn ui-btn-link" href="agent_list.php?set_filter=Y&adm_filter_applied=0&find='.$f_AGENT_ID.'&find_type=id&find_module_id='.$module_id.'&lang='.LANG.'" target="_blank"><span>'.Loc::getMessage("DATA_IMPORT_AGENT_OPEN").'</span></a>';
			$row->AddViewField("AGENT_ID", $agentViewField);
		}
		else
			$row->AddViewField("AGENT_ID", '<img src="/bitrix/panel/'.$module_id.'/theme/images/red.gif" title="'.Loc::getMessage("DATA_IMPORT_AGENT_DELETED").'" />');
	}
	elseif($f_ACTIVE == 'Y')
		$row->AddViewField("AGENT_ID", '<img src="/bitrix/panel/'.$module_id.'/theme/images/red.gif" title="'.Loc::getMessage("DATA_IMPORT_AGENT_DELETED").'" />');
	else
		$row->AddViewField("AGENT_ID", '<img src="/bitrix/panel/'.$module_id.'/theme/images/grey.gif" title="'.Loc::getMessage("DATA_IMPORT_NO_AGENT").'" />');
	...
	$arActions[] = array(
		...
		"ACTION"=>$lAdmin->ActionRedirect("data.import_plan_edit.php?ID=".$f_ID.'&lang='.LANG)
	);
	...
	if($moduleAccessLevel=="W")
	{
		$arActions[] = array(
			...
			"ACTION"=>$lAdmin->ActionRedirect("data.import_plan_edit.php?COPY_ID=".$f_ID.'&lang='.LANG)
		);
	}
	...
	$arActions[] = array(
		...
		"LINK"=>"data.import_plan_connections_edit.php?ID=".$f_ID.'&lang='.LANG,
	);
	...
	$arActions[] = array(
		...
		"LINK"=>"data.import_connections.php?PLAN_ID=".$f_ID."&find_plan_id=".$f_ID."&apply_filter=Y&lang=".LANG,
	);
	...
	if($moduleAccessLevel=="W")
	{
		$arActions[] = array(
			...
			"LINK"=>"data.import_connections_import.php?PLAN_ID=".$f_ID.'&lang='.LANG,
		);
