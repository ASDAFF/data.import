...
if($_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST["Import"]=="Y")
{
	...
	else
	{
		if(!$PLAN_ID)
		{
			$arErrors[] = Loc::getMessage("DATA_IMPORT_PLAN_ID_ERROR")."<br />";
		}
		else
		{
			...
			{
				$arErrors[] = Loc::getMessage("DATA_IMPORT_PLAN_ID_SHOW_IN_MANUALLY_ERROR")."<br />";
				unset($planParams);
			}
			else
			{
				if($LOAD_FILES == "Y")
					CDataImport::Load($PLAN_ID);
			...
			if($planParams)
			{
				...
				{
					$arErrors[] = Loc::getMessage("DATA_IMPORT_INCORRECT_IMPORT_FORMAT")."<br />";
				}
				elseif(!is_file($IMPORT_FILE) && $planParams["IMPORT_FORMAT"] != 'JSON')
				{
					$arErrors[] = Loc::getMessage("DATA_IMPORT_IMPORT_FILE_ERROR")."<br />";
				}
				elseif($connections_count == 0)
				{
					$arErrors[] = Loc::getMessage("DATA_IMPORT_IMPORT_NO_ACTIVE_CONNECTIONS")."<br />";
				}
				else
				{
					...
					switch($planParams["IMPORT_FORMAT"])
					{
						...
						case("XML"):
							...
							/*if(!$scriptData->isValidate())
								$arErrors[] = Loc::getMessage('DATA_IMPORT_ERROR_XML_FILE_IS_INVALID');*/
							break;
						case("XLS"):
					...
					if($planParams["IMPORT_FILE_CHECK"] != "N" && $entities)
					{
						if($planParams["IMPORT_FORMAT"] == "XML")
						{
							if($planParams["XML_USE_ENTITY_NAME"] != "Y")
								$checkEntitiesResult = CDataImport::CheckEntitiesNames($PLAN_ID, $entities["KEYS"]);
						}
						else
							$checkEntitiesResult = CDataImport::CheckEntitiesNames($PLAN_ID, $entities);
						
						if(isset($checkEntitiesResult) && is_array($checkEntitiesResult) && count($checkEntitiesResult))
							$arErrors[] = Loc::getMessage("DATA_IMPORT_ENTITIES_AND_NAMES_NOT_IDENTICAL", ["#ENTITIES#" => implode(', ', $checkEntitiesResult)])."<br />";
					}
					
					$TOTAL = $fileData["ITEMS_COUNT"];
					if($TOTAL == 0)
						$arErrors[] = Loc::getMessage('DATA_IMPORT_ERROR_FILE_EMPTY');
	...
	{
		$arErrors[] = Loc::getMessage("DATA_IMPORT_ERROR_SESSION_EXPIRED")."<br />";
	}
	...
	if(count($arErrors) == 0)
	{
		...
		//$compareText = "CDataImport::Import({$PLAN_ID}, 0);";
		$import = CDataImport::Import($PLAN_ID, $currentItem, false, $NS['IMPORT_FILE']);
		
		preg_match('/CDataImport::Import\([0-9]*\,\s(.*)\);/', $import, $matches);
...
$APPLICATION->SetTitle( Loc::getMessage("DATA_IMPORT_MANUALLY_PAGE_TITLE") );
...
$aTabs = array(
	array(
		"DIV" => "FORM",
		"TAB" => Loc::getMessage("DATA_IMPORT_MANUALLY_TAB_NAME"),
		"ICON" => "",
		"TITLE" => Loc::getMessage("DATA_IMPORT_MANUALLY_TAB_DESCRIPTION")
	),
);
...
function DoNext(NS)
{
	...
	if(running)
	{
		...
		BX.ajax({
			url: 'data.import_manually.php?'+queryString,
			method: 'POST',
			dataType: 'html',
			data: NS,
			...
			onfailure: function(result){
				...
				BX.UI.Notification.Center.notify({
					content: '<?=Loc::getMessage("DATA_IMPORT_ERROR_RESPONSE_STATUS")?>',
					position: "top-right"
				});
			}
		});
...
function EndImport()
{
	...
	window.history.pushState("data_import_manually", "", "data.import_manually.php?lang=<?=LANG?>" );
}
BX.ready(function(){
...
$arFields["IMPORT"]["ITEMS"][] = Array(
	...
	"REQUIRED" => "Y",
	"TYPE" => "SELECT",
	"LABEL" => Loc::getMessage("DATA_IMPORT_PLAN_ID"),
	"ITEMS" => $listPlans,
...
$arFields["IMPORT"]["ITEMS"][] = Array(
	...
	"ID" => "START_FROM",
	"TYPE" => "NUMBER",
	"LABEL" => Loc::getMessage("DATA_IMPORT_START_FROM"),
	"DESCRIPTION" => Loc::getMessage("DATA_IMPORT_START_FROM_DESCRIPTION"),
	"VALUE" => intval($START_FROM),
	"PARAMS" => Array(
		"MIN" => 0
	),
);
$arFields["FILES"]['LABEL'] = Loc::getMessage("DATA_IMPORT_FILES");
$arFields["FILES"]["ITEMS"][] = Array(
	"CODE" => "LOAD_FILES",
	"ID" => "LOAD_FILES",
	"TYPE" => "CHECKBOX",
	"LABEL" => Loc::getMessage("DATA_IMPORT_LOAD_FILES"),
	"DESCRIPTION" => Loc::getMessage("DATA_IMPORT_LOAD_FILES_DESCRIPTION"),
	"VALUE" => $LOAD_FILES,
);
$arFields["FILES"]["ITEMS"][] = Array(
	"CODE" => "UPLOAD_FILE",
	"ID" => "UPLOAD_FILE",
	"TYPE" => "FILE_DIALOG",
	"LABEL" => Loc::getMessage("DATA_IMPORT_IMPORT_FILE"),
	"DESCRIPTION" => Loc::getMessage("DATA_IMPORT_IMPORT_FILE_DESCRIPTION"),
	"PARAMS" => Array(
		"SIZE" => 30,
		"MAXLENGTH" => 255,
		"FORM_NAME" => "data_import_manually",
		"SELECT" => "F",
		"ALLOW_UPLOAD" => "Y",
...
$tabControl->Buttons();
?>
	<button type="button" id="start_button" value="Y" OnClick="StartImport();" class="ui-btn ui-btn-success ui-btn-icon-start"<?=count($listPlans)==0 || $moduleAccessLevel < 'T'?' disabled=""':''?>><?echo Loc::getMessage("DATA_IMPORT_MANUALLY_START")?></button>
	<button class="ui-btn ui-btn ui-btn-icon-stop" type="button" id="stop_button" value="Y" OnClick="EndImport();" disabled><?echo Loc::getMessage("DATA_IMPORT_MANUALLY_STOP")?></button>
<?
$tabControl->End();
