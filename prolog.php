<?
define("ADMIN_MODULE_NAME", "data.import");

$moduleAccessLevel = $APPLICATION->GetGroupRight(ADMIN_MODULE_NAME);

if ($moduleAccessLevel > "D")
{
	if(!CModule::IncludeModule("data.core"))
	{
		$APPLICATION->IncludeAdminFile("data.core", $_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/".ADMIN_MODULE_NAME."/install/data.core.php");
	}
...
$GLOBALS['APPLICATION']->SetAdditionalCSS('/bitrix/panel/data.core/select2/css/select2.min.css');
$GLOBALS['APPLICATION']->SetAdditionalCSS('/bitrix/panel/data.core/select2/css/style.css');
$GLOBALS['APPLICATION']->SetAdditionalCSS('/bitrix/panel/data.core/ui/css/jquery-ui.min.css');
$GLOBALS['APPLICATION']->SetAdditionalCSS('/bitrix/panel/data.core/ui/css/jquery-ui.theme.min.css');

$GLOBALS['APPLICATION']->AddHeadScript('/bitrix/panel/data.core/select2/js/select2.min.js');
if(defined("BX_UTF") || (defined("BX_UTF") && BX_UTF))
	$GLOBALS['APPLICATION']->AddHeadScript('/bitrix/panel/data.core/select2/js/ru.js');
$GLOBALS['APPLICATION']->AddHeadScript('/bitrix/panel/data.core/select2/js/main.js');
$GLOBALS['APPLICATION']->AddHeadScript('/bitrix/panel/data.core/ui/js/jquery-ui.min.js');
$GLOBALS['APPLICATION']->AddHeadScript('/bitrix/panel/data.core/main.js');
...
if ($action=='plan_import' && !$_FILES['xml_file']['error'])
{
	...
	if (file_exists($xmlPath) && CModule::IncludeModule(ADMIN_MODULE_NAME))
	{
		...
		if ($xml->Load($xmlPath))
		{
			if ($node = $xml->SelectNodes('/data_import/params/'))
			{
				$arFields = array();
			...
			if ($newPlanID>0)
			{
				if ($import_iblock_id>0)
				{
					...
					if ($node = $xml->SelectNodes('/data_import/properties/'))
					{
						$arProperties = Array();
					...
					if ($node = $xml->SelectNodes('/data_import/properties_offer/'))
					{
				...
				if ($node = $xml->SelectNodes('/data_import/processing_settings/'))
				{
					$arSettings = Array();
					$cProcessingData = new CDataImportProcessingSettings;
					foreach ($node->children() as $child)
				...
				if ($node = $xml->SelectNodes('/data_import/connections/'))
				{
					$CConnection = new CDataImportPlanConnections;
					foreach ($node->children() as $child)
					{
		...
		if($newPlanID>0)
			LocalRedirect('/bitrix/admin/data.import_plan_edit.php?ID='.$newPlanID.'&lang='.LANGUAGE_ID);
		else
			LocalRedirect('/bitrix/admin/data.import_plan_edit.php?import_status=error&lang='.LANGUAGE_ID);
	}
}
?>