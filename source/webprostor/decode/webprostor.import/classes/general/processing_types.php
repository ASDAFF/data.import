...
class CWebprostorImportProcessingSettingsTypes
{
	...
	public static function ApplyProcessingRules(&$field, $rules)
	{
		...
		$cProcessingSettingsData = new CWebprostorImportProcessingSettings;
		if(count($rules))
		{
				...
				if($ruleArr["ACTIVE"] == "Y")
				{
					...
					switch($function)
					{
						...
						case("lcfirst"):
							...
							break;
						case("ucfirst"):
							$field = CWebprostorCoreFunctions::mb_ucfirst($field);
							break;
						case("abs"):
						case("ceil"):
						case("floor"):
