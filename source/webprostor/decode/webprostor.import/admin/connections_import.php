...
if($back_url=='')
{
	...
	$back_url = '/bitrix/admin/webprostor.import_plan_edit.php?ID='.$PLAN_ID.'&lang='.$lang;
}
				...
				foreach($entitiesArray as $ENTITY => $NAME)
				{
					...
					if($compareProperties)
					{
						if($UPPERCASE_FIRST_SYMBOL == 'Y')
							$NAME = CWebprostorCoreFunctions::mb_ucfirst($NAME);
			...
			if(strlen($apply)<=0 && strlen($strWarning)==0)
			{
				if(strlen($back_url)>0)
				{
					...
					if($REDIRECT_AFTER_CONNECTIONS_IMPORT == "table")
						LocalRedirect('/bitrix/admin/webprostor.import_plan_connections_edit.php?ID='.$PLAN_ID.'&lang='.$lang);
					else
...
<form method="POST" id="connections_import" name="CONNECTIONS_IMPORT" action="webprostor.import_connections_import.php?lang=<?echo LANG?>">
<?=bitrix_sessid_post()?>
...
<?
	...
	echo Loc::getMessage("WEBPROSTOR_IMPORT_CONNECTIONS_IMPORT_NOTE");
