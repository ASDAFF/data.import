...
if($_SERVER["REQUEST_METHOD"] == "POST" && ($_REQUEST["LoadProperties"]=="Y" || $_REQUEST["Import"]=="Y" || $_REQUEST["LoadValues"]=="Y"))
{
	...
	if($_REQUEST["Import"]=="Y")
	{
		...
		else
		{
			if(!$IBLOCK_ID)
			{
				$arErrors[] = Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_ERROR_IBLOCK_ID")."<br />";
			}
			elseif(!$PROPERTY_ID)
			{
				$arErrors[] = Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_ERROR_PROPERTY_ID")."<br />";
			}
			elseif(!$LIST_VALUES)
			{
				$arErrors[] = Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_ERROR_LIST_VALUES")."<br />";
			}
			else
			{
				$ITEMS = explode("\r\n", $LIST_VALUES);
		...
		{
			$arErrors[] = Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_ERROR_SESSION_EXPIRED")."<br />";
		}
		...
		if(count($arErrors) == 0)
		{
			...
			if($CASE_FIRST == 'U')
				$value = CWebprostorImportUtils::mb_ucfirst($value);
			elseif($CASE_FIRST == 'L')
				$value = CWebprostorImportUtils::mb_lcfirst($value);
			
			$ENUM_ID = CWebprostorImportProperty::GetPropertyIdByValue($IBLOCK_ID, $PROPERTY_ID, $value, $NS["PARAMS"], $sort);
			if($ENUM_ID)
				$NS['ID'][] = $ENUM_ID;
			...
			if($NS["CURRENT"] < $NS["TOTAL"])
			{
				...
				CWebprostorCoreFunctions::showProgressBar(
					'default', 
					Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_PROGRESS", array(
						"#DONE#" => number_format(intVal($NS["CURRENT"]), 0, "", " "),
						"#TOTAL#"=> number_format(intVal($NS["TOTAL"]), 0, "", " "),
					)),
					Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_PROGRESS_STEP", array(
			...
			else
			{
				...
				$arMessages[] = Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_PROGRESS_FINISH", array(
					"#TOTAL#"=> number_format(intVal($NS["TOTAL"]), 0, "", " "),
					"#TIME_MIN#"=> $TIME_MIN,
				...
				if($DELETE_OTHER_VALUES == 'Y')
					CWebprostorImportProperty::DeleteValues($IBLOCK_ID, $PROPERTY_ID, $NS['ID']);
	...
	elseif($_REQUEST["LoadProperties"]=="Y" && check_bitrix_sessid())
	{
		...
		{
			echo \Bitrix\Main\Web\Json::encode(Loc::getMessage('WEBPROSTOR_IMPORT_LIST_VALUES_PROPERTY_ID_EMPTY'));
		}
	}
	elseif($_REQUEST["LoadValues"]=="Y" && check_bitrix_sessid())
	{
		...
		{
			echo \Bitrix\Main\Web\Json::encode(Loc::getMessage('WEBPROSTOR_IMPORT_LIST_VALUES_LOAD_VALUES_EMPTY'));
		}
	}
...
$APPLICATION->SetTitle( Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_PAGE_TITLE") );
...
$aTabs = array(
	array(
		"DIV" => "FORM",
		"TAB" => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_TAB_NAME"),
		"ICON" => "",
		"TITLE" => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_TAB_DESCRIPTION")
	),
);
...
$(document).ready(function(){
	...
	function loadValues(IBLOCK_ID, PROPERTY_ID)
	{
		if(IBLOCK_ID>0 && PROPERTY_ID>0)
		{
			...
			$.ajax({
				url: 'webprostor.import_list_values.php',
				method: 'post',
				dataType: 'json',
				data: {
					lang: '<?echo LANG?>',
...
function DoNext(NS)
{
	...
	if(running)
	{
		ShowWaitWindow();
		BX.ajax.post(
			'webprostor.import_list_values.php?'+queryString,
			NS,
			function(result){
...
function EndImport()
{
	...
	window.history.pushState("webprostor_import_list_values", "", "webprostor.import_list_values.php?lang=<?=LANG?>" );
}
BX.ready(function(){
...
$arFields["IBLOCK"]["ITEMS"][] = Array(
	...
	"TYPE" => "IBLOCK",
	"REQUIRED" => "Y",
	"LABEL" => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_IBLOCK_ID"),
	"VALUE" => $IBLOCK_ID,
	"PARAMS" => Array(
		"MIN_PERMISSION" => "W"
	),
...
$arFields["IBLOCK"]["ITEMS"][] = Array(
	...
	"LABEL" => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_PROPERTY_ID"),
	"DESCRIPTION" => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_PROPERTY_ID_EMPTY"),
	"PARAMS" => Array(
		"IBLOCK_ID" => $IBLOCK_ID, 
		"PROPERTY_TYPE" => "L"
	),
...
$arFields["IBLOCK"]["ITEMS"][] = Array(
	...
	"LABEL" => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_LOAD_VALUES"),
	"VALUE" => 'javascript:;',
	"PARAMS" => Array(
		"TEXT" => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_LOAD_VALUES_TEXT"),
		"TARGET" => "_self",
	),
);
$arFields["IBLOCK"]["ITEMS"][] = Array(
	...
	"REQUIRED" => "Y",
	"DISABLED" => $PROPERTY_ID ? "N" : "Y",
	"LABEL" => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_LIST_VALUES"),
	"PARAMS" => Array(
		"PLACEHOLDER" => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_LIST_VALUES_PLACEHOLDER"),
		"COLS" => 60,
		"ROWS" => 10,
	),
	"VALUE" => $LIST_VALUES,
);
$arFields["PARAMS"]["LABEL"] = Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_OPTIONS");
$arFields["PARAMS"]["ITEMS"][] = Array(
	"CODE" => "CASE_FIRST",
	"TYPE" => "SELECT",
	"LABEL" => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_CASE_FIRST"),
	"ITEMS" => [
		'' => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_CASE_FIRST_NO"),
		'U' => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_CASE_FIRST_U"),
		'L' => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_CASE_FIRST_L"),
	],
	"PARAMS" => Array(
		"PLACEHOLDER" => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_PROPERTY_ID_EMPTY"),
	),
	"VALUE" => $CASE_FIRST,
);
$arFields["PARAMS"]["ITEMS"][] = Array(
	"CODE" => "SET_SORT",
	"TYPE" => "CHECKBOX",
	"LABEL" => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_SET_SORT"),
	"DESCRIPTION" => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_SET_SORT_NOTE"),
	"VALUE" => $SET_SORT ? $SET_SORT : "N",
);
$arFields["PARAMS"]["ITEMS"][] = Array(
	"CODE" => "UPDATE_VALUES",
	"TYPE" => "CHECKBOX",
	"LABEL" => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_UPDATE_VALUES"),
	"DESCRIPTION" => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_UPDATE_VALUES_NOTE"),
	"VALUE" => $UPDATE_VALUES ? $UPDATE_VALUES : "N",
);
$arFields["PARAMS"]["ITEMS"][] = Array(
	"CODE" => "DELETE_OTHER_VALUES",
	"TYPE" => "CHECKBOX",
	"LABEL" => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_DELETE_OTHER_VALUES"),
	"DESCRIPTION" => Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_DELETE_OTHER_VALUES_NOTE"),
...
?>
	<button type="button" id="start_button" value="Y" OnClick="StartImport();" class="ui-btn ui-btn-success ui-btn-icon-start" disabled>
		<?echo Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_START")?>
	</button>
	<button class="ui-btn ui-btn ui-btn-icon-stop" type="button" id="stop_button" value="Y" OnClick="EndImport();" disabled>
		<?echo Loc::getMessage("WEBPROSTOR_IMPORT_LIST_VALUES_STOP")?>
	</button>
<?
$tabControl->End();
?>
