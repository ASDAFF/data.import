...
class CWebprostorImportPlan
{
	var $MODULE_ID = "webprostor.import";
	
	var $DB_NAME = "webprostor_import_plans";
	var $LAST_ERROR = "";
	var $LAST_MESSAGE = "";
	...
	public function AddAgent($ID, $AGENT_INTERVAL, $TYPE = "Import", $TIME = false)
	{
		...
		if(!$TYPE)
			$result = $DB->Update("webprostor_import_plans", Array("AGENT_ID" => $AGENT_ID), "WHERE ID='".$ID."'", $err_mess.__LINE__, false, true);
	}
	...
	public function Delete($ID)
	{
		...
		if($res)
		{
			...
			$CConnections = new CWebprostorImportPlanConnections;
	...
	{
		return [
			"CSV" => Loc::getMessage("WEBPROSTOR_IMPORT_FORMAT_CSV").' [CSV]',
			"XML" => Loc::getMessage("WEBPROSTOR_IMPORT_FORMAT_XML").' [XML]',
			"XLS" => Loc::getMessage("WEBPROSTOR_IMPORT_FORMAT_XLS").' [XLS]',
			"XLSX" => Loc::getMessage("WEBPROSTOR_IMPORT_FORMAT_XLSX").' [XLSX]',
			"ODS" => Loc::getMessage("WEBPROSTOR_IMPORT_FORMAT_ODS").' [ODS]',
			"XODS" => Loc::getMessage("WEBPROSTOR_IMPORT_FORMAT_XODS").' [XODS]',
			"JSON" => Loc::getMessage("WEBPROSTOR_IMPORT_FORMAT_JSON").' [JSON]',
		];
	}
}
?>